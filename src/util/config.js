export const simpleRandom = 5;
export const timeLocation = 8;
export const cluster = 9;
export const systematicRandom = 10;
export const api = 'https://ifrc-api-free.vercel.app/api'
export const toolLink = 'https://ifrc-sampling.vercel.app/'
export const IFRCLink = 'https://go.ifrc.org'
export const startId = 1;
export const household = 'Households';
export const subgroup = 'sub-population groups';
export const isSubgroup = 'Yes';

export const config = {
    simpleRandom,
    timeLocation,
    cluster,
    systematicRandom,
    api,
    toolLink,
    IFRCLink,
    startId,
    household,
    subgroup,
    isSubgroup
};

export default config